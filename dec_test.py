...
if __name__ == "__main__":
    ...
    oct_normal_tensor = utils.external_dimension_creator(2018, 12, 31, "20181201", max_temps, min_temps, solar_exps,
                                                         rainfalls)

    external_dimension_for_day = oct_normal_tensor[504:]

    # Create Data for ST CNN part
    matrix_data_10 = utils.data_deal_cnn('data/result_dec.csv', 31, '0')
    ...
    print('build a mask')
    # build a mask
    data_oct = pd.read_csv('data/result_dec.csv')
    ...
    # save all stops list
    with open('data/BSID_top_list_december.txt', 'wt') as f:
    ...
    with open('data/ASID_top_list_december.txt', 'wt') as f:
    ...
    # save top 16 lists
    with open('data/BSID_top_list_10_december.txt', 'wt') as f:
        for stoplist in stop_distribution_BSID:
            for i in stoplist:
                line = str(i) + ' '
                f.write(line)
    ...
    with open('data/ASID_top_list_10_december.txt', 'wt') as f:
        for stoplist in stop_distribution_ASID:
            for i in stoplist:
                line = str(i) + ' '
                f.write(line)
    ...
    matrix_data_10, ground_truth_10 = utils.data_deal_gnn('data/result_dec.csv', 31,
                                                          stop_distribution,
    ...
    gCNN = modules.GCModel(mask_in, mask_out, stop_distribution, stop_number_location, (3, 2, rows, 10))
    ...
    for batch in train_loader:
        ...
        loss = torch.zeros(1)
        for i in range(20):
            print('batch', str(j), 'data number', str(i))
    ...
    print('------------')
    t = output[0][0] * (max_in - min_in) + min_in
